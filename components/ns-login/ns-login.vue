			...
			<uni-popup ref="auth" :custom="true" :mask-click="false">
				<view class="uni-tip">
					...
					<view class="uni-tip-group-button">
						...
						<button v-if="canIUseGetUserProfile" type="primary" class="uni-tip-button" @click="login">立即登录</button>
						<button v-else type="primary" open-type="getUserInfo" @getuserinfo="login" class="uni-tip-button">立即登录</button>
						<!-- #endif  -->
						<!-- #ifdef MP-ALIPAY -->
...
export default {
	...
	methods: {
		 ...
		 */
		toLogin() {
			if (this.url) this.$util.redirectTo('/otherpages/login/login/login', { back: encodeURIComponent(this.url) });
