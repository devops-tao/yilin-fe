		...
		<view @touchmove.prevent.stop v-if="newgift" class="reward-popup">
			<uni-popup ref="nsNewGift" type="center" :maskClick="false">
				<view class="reward-wrap">
					<view class="newgift-content" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/holiday_polite-bg.png') + ')' }">
						<view class="content-title-holiday">
							<image :src="$util.img('upload/uniapp/holiday_polite_left.png')" mode="" class="birthday-img-all"></image>
							<view class="font-size-toolbar activity-name">{{ newgift.activity_name }}</view>
							<image :src="$util.img('upload/uniapp/holiday_polite_right.png')" mode="" class="birthday-img-all"></image>
						</view>
						...
						<scroll-view scroll-y="true" class="register-box">
							<view :class="introduction>38?'reward-content':'reward-content-two'" >
								<view class="content" v-if="newgift.award_list.point > 0">
									<view class="info">
											<text class="num">{{ newgift.award_list.point }}<text class="type">积分</text></text>
								...
								<view class="content" v-if="newgift.award_list.balance_type == 0 && newgift.award_list.balance > 0">
									<view class="info">
											<text class="num">{{ newgift.award_list.balance | int}}<text class="type">元红包</text></text>
								...
								<view class="content" v-if="newgift.award_list.balance_type == 1 && newgift.award_list.balance_money > 0">
									<view class="info">
											<text class="num">{{ newgift.award_list.balance_money | int}}<text class="type">元红包</text></text>
								...
								<block v-if="newgift.award_list.coupon_list.length > 0">
									<block v-for="(item, index) in newgift.award_list.coupon_list" :key="index">
										<view class="content">
											<view class="info">
												<text v-if="item.type == 'reward'" class="num">{{ parseFloat(item.money) }}<text class="type">元优惠劵</text></text>
												<text v-else-if="item.type == 'discount'" class="num">{{ item.discount | int}}<text class="type">折</text></text>
