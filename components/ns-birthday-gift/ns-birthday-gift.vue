		...
		<view @touchmove.prevent.stop v-if="birthday" class="reward-popup">
			<uni-popup ref="birthdayGift" type="center" :maskClick="false">
				<view class="reward-wrap">
					<view class="wrap" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/birthday_gift_bg.png') + ')' }">
						...
						<view class="birthday-title-hint">
							<image :src="$util.img('upload/uniapp/birthday_gift_left.png')" mode="" class="birthday-img-all"></image>
							<view class="font-size-toolbar">生日贺礼</view>
							<image :src="$util.img('upload/uniapp/birthday_gift_right.png')" mode="" class="birthday-img-all"></image>
						</view>
						<scroll-view scroll-y="true" class="register-box">
							<view class="reward-content">
								<view class="content"  v-if="birthday.point > 0">
									<view class="info">
										<text class="num">{{parseFloat(birthday.point)}}<text class="type">积分</text></text>
								...
								<view class="content" v-if="birthday.balance_type == 0 && birthday.balance > 0">
									<view class="info">
										<text class="num">{{parseFloat(birthday.balance)}}<text class="type">元红包</text></text>
								...
								<view class="content" v-if="birthday.balance_type == 1 && birthday.balance_money > 0">
									<view class="info">
										<text class="num">{{parseFloat(birthday.balance_money)}}<text class="type">元红包</text></text>
										...
										<view class="content">
											<view class="info">
												<block v-if="item.type == 'reward'">
													<text class="num">{{parseFloat(item.money)}}<text class="type">元优惠劵</text></text>
												</block>
												<block v-else-if="item.type == 'discount'">
													<text class="num">{{item.discount}}<text class="type">折</text></text>
												</block>
												<view class="desc">用于下单时抵现或兑换商品等</view>
											</view>
