<template>
	<view class="category-level">
					...
					<scroll-view scroll-y="true" :style="{ height: 'calc(100% - ' + height + ' )' }">
						...
						>
							<view class="twoScroll-goods" v-for="(item, index) in cateList[oneIndex].child_list[twoIndex].child_list" :key="index">
								<view class="twoScroll-goods-title" @click="toListDetail(item.category_id_3)">
									<text class="goods-title">{{ item.category_name }}</text>
									<view class="see-more">
										<text class="more-tip">{{ $lang('seeMore') }}</text>
										<text class="more-more iconfont iconright"></text>
									</view>								
								</view>
		...
		<ns-empty v-else :fixed="!1" :isIndex="!1" text="暂无商品分类"></ns-empty>
...
export default {
	...
	computed: {
		flRefresh() {
			return this.$store.state.flRefresh;
		}
	},
	watch: {
		flRefresh: function(nval, oval) {
			this.isNetwork = 1;
			this.isAll = false;
