		...
		<block v-for="(item, index) in formData">
			<view class="order-wrap" v-if="item.controller == 'Text'">
				<view class="order-cell">
					<view class="name">
						<text class="required" v-if="item.value.required">*</text>
			...
			<view class="order-wrap" v-if="item.controller == 'Textarea'">
				<view class="order-cell clear-flex">
					<view class="name">
						<text class="required" v-if="item.value.required">*</text>
						<text class="tit">{{ item.value.title }}</text>
					</view>
					...
					<view class="box">
						<checkbox-group @change="checkboxChange($event, index)">
							<label v-for="(v, k) in item.option_lists" :key="k">
								<checkbox :value="v.value" :checked="v.checked" /> {{v.value}}
							</label>
						</checkbox-group>
					</view>
				</view>
				...
				<view class="order-cell clear-flex">
					...
					<view class="box">
						<radio-group @change="radioChange($event, index)">
							<label class="uni-list-cell uni-list-cell-pd" v-for="(v, k) in item.option_lists" :key="k">
								<view>
									<radio :value="v.value" :checked="v.checked" /> {{v.value}}
								</view>
							</label>
						</radio-group>
					</view>
			...
			<view class="order-wrap" v-if="item.controller == 'Img'">
				<view class="order-cell clear-flex">
					...
					<view class="box img-boxs">
						...
						<view class="img-box" @click="addImg(index)">
			...
			<view class="order-wrap" v-if="item.controller == 'City'">
				<view class="order-cell clear-flex">
					...
					<view class="box">
						<pick-regions :default-regions="defaultRegions" :select-arr="item.select_arr" @getRegions="handleGetRegions($event, index)">
							<text class="select-address " :class="{ empty: !item.val, 'color-tip': !item.val }">
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 修改图片
			uploadImg(index) {
				this.$util.upload(Number(this.formData[index].value.max_count), {
					path: 'evaluateimg'
				}, res => {
					if (res.length > 0) {
						res.forEach((v) => {
