	...
	methods: {
		...
		pay() {
			...
			if(this.payInfo.event == 'BlindboxGoodsOrderPayNotify'){
				return_url = '/otherpages/blindBox/index/index?outTradeNo=';
			}else{
				return_url = '/otherpages/pay/result/result?code=';
			}
			this.$api.sendRequest({
				url: '/api/pay/pay',
				data: {
						...
						switch (payType.type) {
							case 'alipay':
								if(this.$util.isWeiXin()){
									...
									this.$util.redirectTo('/otherpages/pay/wx_pay/wx_pay', { wx_alipay: wx_alipay, out_trade_no: this.payInfo.out_trade_no }, '', 'redirectTo');
								}else{
											...
											wxJS.pay(
												...
												res => {
													if (res.errMsg == 'chooseWXPay:ok') {
														if(this.payInfo.event == 'BlindboxGoodsOrderPayNotify'){
															this.$util.redirectTo('/otherpages/blindBox/index/index', { outTradeNo: this.payInfo.out_trade_no }, '', 'redirectTo');
														}else{
															this.$util.redirectTo('/otherpages/pay/result/result', { code: this.payInfo.out_trade_no }, '', 'redirectTo');
														}
