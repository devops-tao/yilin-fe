export default {
	...
	methods: {
		...
		selectAddress() {
			this.$util.redirectTo('/otherpages/member/address/address', {
				'back': '/promotionpages/bale/payment/payment'
			});
		},
		/**
		 * 获取订单初始化数据
		 */
		getOrderPaymentData() {
			this.orderCreateData = uni.getStorageSync('baleOrderCreateData');
			// 获取经纬度
			...
			this.$api.sendRequest({
				url: '/bale/api/ordercreate/payment',
		...
		orderCalculate() {
			...
			this.$api.sendRequest({
				url: '/bale/api/ordercreate/calculate',
				data,
				success: res => {
					if (res.code >= 0) {
		...
		openChoosePayment() {
			...
			this.$api.sendRequest({
				url: '/bale/api/ordercreate/verifyarea',
				data,
				success: res => {
					if (res.code >= 0) {
		...
		orderCreate() {
			if (this.verify()) {
				...
				this.$api.sendRequest({
					url: '/bale/api/ordercreate/create',
					data,
					success: res => {
						if (res.code >= 0) {
							uni.removeStorage({
								...
								success: () => {
									if (this.orderPaymentData.pay_money == 0) {
										...
										let create = uni.getStorageSync('orderCreateData');
										create.out_trade_no = res.data;
										uni.setStorageSync('orderCreateData', create);
	...
	onShow() {
		...
		if(uni.getStorageSync('orderCreateData') && uni.getStorageSync('orderCreateData').out_trade_no){
