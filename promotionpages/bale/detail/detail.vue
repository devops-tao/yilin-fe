<template>
	<view class="container" :data-theme="themeStyle">
		<mescroll-uni top="0" ref="mescroll" @getData="getGoodsList" v-if="info">
			<block slot="list">
				...
				<view class="activity-head color-base-bg">
					...
					<view class="time" v-if="timeMachine">距离结束还剩
						<uni-count-down
							:day="timeMachine.d"
							:hour="timeMachine.h"
				...
				<view class="goods-wrap" >
					<view class="goods-item" v-for="(item, index) in goodsList" :key="index">
						<view class="goods-image" @click="$util.redirectTo('/pages/goods/detail/detail', {sku_id: item.sku_id})">
							<image :src="$util.img(item.sku_image, { size: 'mid' })" mode="widthFix" @error="imageError(index)"></image>
						</view>
						<view class="goods-info">
							<view class="name" @click="$util.redirectTo('/pages/goods/detail/detail', {sku_id: item.sku_id})">{{ item.goods_name }}</view>
							<view class="introduction">{{ item.introduction }}</view>
							<view class="goods-bottom">
								<view class="price color-base-text font-size-toolbar">
		...
		<block  v-if="info">
			<view class="footer-wrap">
				<view class="left">
					...
					<view class="data">
						<view class="price color-base-text"><text class="unit">{{ $lang('common.currencySymbol') }}</text>{{ totalPrice|moneyFormat }}</view>
			...
			<view class="cart-popup" :class="{show: cartShow}" v-if="skuList.length">
				<view class="header">
					...
					<view class="right" @click="clearCart"><text class="iconfont iconicon7"></text><text>清空购物车</text></view>
				</view>
		...
		<ns-goods-sku ref="goodsSku" @refresh="refreshGoodsSkuDetail" @confirm="joinCart" :goods-detail="goodsSkuDetail" v-if="goodsSkuDetail"></ns-goods-sku>
