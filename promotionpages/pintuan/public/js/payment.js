export default {
	...
	methods: {
		...
		createBtn(){
			...
			&& this.orderPaymentData.shop_goods_list.delivery.start_money > this.orderPaymentData.shop_goods_list.goods_money){
				return false;
			}
			if(this.orderPaymentData.delivery.delivery_type == 'local'
			&& this.orderPaymentData.shop_goods_list.delivery 
					...
					success: res => {
						...
						if (res.code >= 0) {
							if (this.orderPaymentData.pay_money == 0) {
								...
								let orderCreateData = uni.getStorageSync('pintuanOrderCreateData');
								orderCreateData.out_trade_no = res.data;
								uni.setStorageSync('pintuanOrderCreateData', orderCreateData);
		...
		closeInvoicePopup() {
			// 发票
			// this.orderCreateData.is_invoice = 0;
			// // console.log(this.orderPaymentData.shop_goods_list.invoice.invoice_type[0],888)
			// this.orderCreateData.invoice_type = this.orderPaymentData.shop_goods_list.invoice.invoice_type[0]; // 发票类型  1 纸质 2 电子
		...
		openChoosePayment() {
			...
			this.$api.sendRequest({
				url: '/pintuan/api/ordercreate/verifyareaorstock',
				data,
				success: res => {
					if (res.code >= 0) {
		...
		localtime(type = ''){
			...
			let data = {
				time_type:this.$util.deepClone(this.orderPaymentData.shop_goods_list.local_config.info).time_type,
				end_time:this.$util.deepClone(this.orderPaymentData.shop_goods_list.local_config.info).end_time,
				start_time:this.$util.deepClone(this.orderPaymentData.shop_goods_list.local_config.info).start_time,
				time_week:this.$util.deepClone(this.orderPaymentData.shop_goods_list.local_config.info).time_week,
			};
			let obj = {
		...
		storetime(type = ''){
			console.log("进入门店自提")
			console.log('this.orderPaymentData.shop_goods_list',this.orderPaymentData.shop_goods_list)
			if(this.orderPaymentData.shop_goods_list.delivery_store_info){
				let delivery_store_info = JSON.parse(this.orderPaymentData.shop_goods_list.delivery_store_info)
				let data = {
	...
	onShow() {
		...
		var orderCreateData = uni.getStorageSync('pintuanOrderCreateData');
