			...
			<view class="layer">
				<view class="popup-center">
					...
					<view class="popup-center-center">
						...
						<view class="popup-center-center-image">
							<image :src="$util.img('/upload/uniapp/default_headimg.png')" mode=""></image>
						</view>
					</view>
					<view class="popup-center-die" @click="joinPintuan()">
						参与拼单
					</view>
					
					<view class="popup-center-delect" @click="close">
						<image :src="$util.img('upload/uniapp/goods/mask_layer_pintuan_close.png')" />
					</view>
				</view>
			</view>
		</uni-popup>
		...
		methods:{
			...
			delegation(){
				if (!uni.getStorageSync('token')) {
					setTimeout(() => {
						this.$refs.login.open('/promotionpages/pintuan/order/order?pintuan_id=' + this.pintuan_id + '&group_id=' + this.groupId + '&id=' + this.id);
					});
					return;
				} 
			...
			pintuan(isPromotion = true) {
				if (!uni.getStorageSync('token')) {
					this.$refs.login.open('/promotionpages/pintuan/detail/detail?pintuan_id=' + this.groupId);
					return;
				}
				if (isPromotion) {
					this.resetPrice();
