			...
			<view class="goods-container">
				...
				>
					<view class="price-info">
						<view class="img-wrap"><image :src="$util.img('upload/uniapp/goods/detail_bargain_tag.png')" mode="aspectFit"></image></view>
		...
		<!-- 添加二维码 end -->		
				...
				<view class="group-wrap" v-if="evaluateConfig.evaluate_show == 1">
					<view class="goods-evaluate" @click="toEvaluateDetail(goodsSkuDetail.goods_id)">
						...
						<view class="evaluate-item" v-for="(item,index) in goodsEvaluate" v-if="item.content">
					...
					<template v-if="goodsSkuDetail.goods_state == 1">
						...
						<block v-if="goodsSkuDetail.launch_info != undefined">
							<ns-goods-action-button
								...
								text="正在砍价"
								@click="$util.redirectTo('/promotionpages/bargain/launch/launch', { id: goodsSkuDetail.launch_info.launch_id })"
							/>
						</block>
						<block v-else>
							<ns-goods-action-button
								class="goods-action-button active4"
								:text-price="'最低¥ ' + goodsSkuDetail.show_price"
								text="去砍价"
								@click="bargain"
								v-if="goodsSkuDetail.bargain_stock > 0 || (goodsSkuDetail.bargain_stock == 0 && goodsSkuDetail.sku_spec_format)"
							/>
