	...
	methods: {
		...
		getOrderPaymentData() {
			...
			this.$api.sendRequest({
				...
				success: res => {
					if (res.code >= 0) {
						//var invoice_type_temp = res.data.shop_goods_list.invoice.invoice_type;
						//res.data.shop_goods_list.invoice.invoice_type = invoice_type_temp.split(',');
		...
		openChoosePayment() {
			...
			this.$api.sendRequest({
				url: '/bargain/api/ordercreate/verifyareaorstock',
				data,
				success: res => {
					if (res.code >= 0) {
