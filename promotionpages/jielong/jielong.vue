<template>
	<view class="body">
					...
					<view class="content">
						...
						<view class="content-num" v-if="item.member_headimg.length > 0">
							<view class="content-num-image">
								<image :src="$util.img(data) ? $util.img(data) : $util.getDefaultImage().default_headimg" mode="" v-for="(data,value) in item.member_headimg.slice(0,3)" :key="value"></image>
							</view>
		...
		<view class="old-buy old-buy-one" v-if="jielong_buy_page.count == 0">
			<view class="head">
				<image :src="$util.img('upload/uniapp/jielong/left.png')" mode=""></image>
				<text class="old-buy-head-text">他们都在买</text>
				<image :src="$util.img('upload/uniapp/jielong/right.png')" mode=""></image>    
			</view>
			<view class="content">
				<image :src="$util.img('upload/uniapp/jielong/wuren.png')" mode=""></image>
			</view>
				...
				<view class="content" v-for="(item, index) in (jielong_buy_page.list || '').slice(0,cutting)" :key="index">
					<view style="display: flex;">
						<view class="old-buy-content">
							<image class="old-buy-content-image" :src="item.headimg=='' ? $util.getDefaultImage().default_headimg : $util.img(item.headimg)"></image>
		...
		<uni-popup ref="cartPopup" type="bottom" class="cart-popup" style="margin-bottom: 100rpx;">
			<view class="popup-shop">
				...
				<view class="cart-contentOne" v-if="returnShoping.length==0">
					<image :src="$util.img('upload/uniapp/jielong/wurenOne.png')" mode=""></image>
				</view>
				...
				<view class="popupContent-top" >
					<view class="popupContent-top-title" >
						...
						<view class="popupContent-top-title-right">
							...
							<view class="popupContent-top-title-right-textBottom">
		...
		<view class="fixed-left" v-if="horseRace">
			<view class="fixed-left-content">
				<image :src="lantern.headimg=='' ? $util.getDefaultImage().default_headimg : $util.img(lantern.headimg)"></image>
		...
		methods: {
			...
			toLogin() {
				if(!uni.getStorageSync('token')){
					this.$refs.login.open('/promotionpages/jielong/jielong?jielong_id=' + this.jielong_id);
					return true
				}
			},
			//商品渲染
			...
			share(){
				...
				this.$api.sendRequest({
					...
					data: {
						page:'/promotionpages/jielong/jielong',
						qrcode_param:id,
					},
					success: res => {
						uni.hideLoading();  
...
<style scoped lang="scss">
	...
	*{
	...
	.topHead{
		width: 100%;
		min-height: 200rpx;
		background: url("https://cs.b2c.niuteam.cn/upload/uniapp/jielong/banyuan.png") no-repeat;
