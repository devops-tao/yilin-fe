		...
		<view class="die">
			...
			<view class="die-content">
				<view class="die-content-image" v-for="(item,index) in groupDetail.member_list" :key="index">
					<image :src="item.member_img ? $util.img(item.member_img) : $util.img('/upload/uniapp/default_headimg.png')" mode=""></image>
				</view>
				<view class="die-content-image" v-for="itm in kill">
					<!-- <image :src="$util.img('/upload/uniapp/default_headimg.png')" mode=""></image> -->
					<image :src=" $util.img('/upload/uniapp/spelling_who.png')" mode="aspectFill"></image>
				</view>
			</view>
		...
		<uni-popup ref="pintuanPopup" type="center" class="pintuan-popup-layer">
			<view class="layer">
				<view class="popup-center">
					...
					<view class="popup-center-delect" @click="close"><image :src="$util.img('upload/uniapp/goods/mask_layer_pintuan_close.png')" /></view>
				</view>
			</view>
		</uni-popup>
		...
		methods:{
			...
			delegation(){
				if (!uni.getStorageSync('token')) {
					setTimeout(() => {
						this.$refs.login.open('/promotionpages/rebate/rebate/order?pinfan_id=' + this.pinfan_id + '&group_id=' + this.groupId + '&id=' + this.id);
					});
					return;
				} 
			...
			async getGoodsSkuDetail() {
				...
				if (data.goods_sku_detail != null) {
					...
					if ((this.goodsSkuDetail.end_time - res.timestamp) > 0) {
						...
						setTimeout(() => {
							this.$util.redirectTo('/pages/goods/detail/detail', {sku_id: this.goodsSkuDetail.sku_id}, 'redirectTo');
						}, 1000);
					}
			...
			pintuan(isPromotion = true) {
				if (!uni.getStorageSync('token')) {
					this.$refs.login.open('/promotionpages/rebate/rebate/detail?pinfan_id=' + this.groupId);
					return;
				}
				if (isPromotion) {
					this.resetPrice();
