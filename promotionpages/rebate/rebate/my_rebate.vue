		...
		<mescroll-uni @getData="getData" top="90" ref="mescroll" :size="10" v-if="token">
			<block slot="list">
				...
				<view v-if="dataList.length == 0" style="padding-top:0">
					<ns-empty text="暂无拼团返利订单" :emptyBtn="emptyBtn"></ns-empty>
				</view>
			</block>
		</mescroll-uni>
	...
	export default {
		...
		data() {
			return {
				...
				emptyBtn: {
					text: '去逛逛',
					url: '/promotionpages/rebate/rebate/list'
				},
				token: null
			};
		},
		...
		onShow() {
			...
			if (uni.getStorageSync('token')) {
				...
				setTimeout(() => {
					this.$refs.login.open('/promotionpages/rebate/rebate/my_rebate');
				});
			}
		},
		methods: {
			...
			toReward(e,related_id){
				if(e==1||e==2){
					this.$util.redirectTo('/otherpages/member/balance_detail/balance_detail', {related_id:related_id,from_type:'pinfan'});
				}else if(e==3){
					this.$util.redirectTo('/otherpages/member/coupon/coupon', {related_id:related_id});
				}else if(e==4){
					this.$util.redirectTo('/otherpages/member/point_detail/point_detail', {related_id:related_id,from_type:'pinfan'});
				}
			},
			//去首页
			goIndex() {
			...
			toshare(id) {
				this.$util.redirectTo('/promotionpages/rebate/rebate/share', {
					id: id
				});
			},
			toOrderDetail(id,delivery_type) {
				if(delivery_type==''){
					this.$util.redirectTo('/otherpages/order/detail_virtual/detail_virtual', { order_id: id });
					...
					this.$util.redirectTo('/pages/order/detail_local_delivery/detail_local_delivery', {order_id: data.order_id});
				}else if(delivery_type=='express'){
					this.$util.redirectTo('/pages/order/detail/detail', {order_id: id});
				}
			},
			//切换分类
			categoryChange(e) {
