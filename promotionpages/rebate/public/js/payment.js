export default {
	...
	methods: {
		...
		selectAddress() {
			var params = {
				back: '/promotionpages/rebate/rebate/payment',
				local: 0,
				type: 1
			}
			// 外卖配送需要定位地址
		...
		// 获取订单初始化数据
		getOrderPaymentData() {
			this.orderCreateData = uni.getStorageSync('pinfanOrderCreateData');
			...
			this.$api.sendRequest({
				url: '/pinfan/api/ordercreate/payment',
		...
		orderCalculate() {
			...
			this.$api.sendRequest({
				url: '/pinfan/api/ordercreate/calculate',
				data,
				success: res => {
					if (res.code >= 0) {
		...
		orderCreate(pay_password) {
			if (this.verify()) {
				...
				this.$api.sendRequest({
					url: '/pinfan/api/ordercreate/create',
					data,
					success: res => {
						uni.hideLoading();
						if (res.code >= 0) {
							if (this.orderPaymentData.pay_money == 0) {
								...
								let orderCreateData = uni.getStorageSync('pinfanOrderCreateData');
								orderCreateData.out_trade_no = res.data;
								uni.setStorageSync('pinfanOrderCreateData', orderCreateData);
		...
		openChoosePayment() {
			...
			this.$api.sendRequest({
				url: '/pinfan/api/ordercreate/verifyareaorstock',
				data,
				success: res => {
					if (res.code >= 0) {
	...
	onShow() {
		...
		var orderCreateData = uni.getStorageSync('pinfanOrderCreateData');
