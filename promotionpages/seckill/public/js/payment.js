export default {
	...
	methods: {
					...
					success: res => {
						...
						if (res.code >= 0) {
							if (this.orderPaymentData.pay_money == 0) {
								...
								let orderCreateData = uni.getStorageSync('seckillOrderCreateData');
								orderCreateData.out_trade_no = res.data;
								uni.setStorageSync('seckillOrderCreateData', orderCreateData);
		...
		openChoosePayment() {
			...
			this.$api.sendRequest({
				url: '/seckill/api/ordercreate/verifyareaorstock',
				data,
				success: res => {
					if (res.code >= 0) {
	...
	onShow() {
		...
		var orderCreateData = uni.getStorageSync('seckillOrderCreateData');
