			...
			<block slot="list">
				<view class="cf-container color-line-border">
					<view class="tab">
						<view @click="changeState(1)"><text
								:class="order_type == 1 ? 'color-base-text active color-base-border-bottom' : ''">全部</text>
						</view>
						<view @click="changeState(2)"><text
								:class="order_type == 2 ? 'color-base-text active color-base-border-bottom' : ''">待支付</text>
						</view>
						<view @click="changeState(3)"><text
								:class="order_type == 3 ? 'color-base-text active color-base-border-bottom' : ''">已完成</text>
						</view>
					</view>
				</view>
					...
					<view  v-for="(item, index) in orderList" :key="index" >
						<view class="card-item" v-if="item.status == 0">
							<view class="card-content">
								<view class=""
									style="display: flex; justify-content: space-between; margin-bottom: 30rpx;border-bottom: 1rpx solid #fcfcfc;">
								...
								<view class="" style="display: flex; justify-content: space-between;">
									<view class="card-use">
										...
										<text class="card-num" v-if="item.balance>0 && item.point>0 && item.goods_num>0">￥{{item.balance}} + {{item.point}}积分+商品*{{item.goods_num}}</text>
										<text class="card-num" v-else-if="item.balance>0 && item.point==0">￥{{item.balance}}</text>
										<text class="card-num" v-else-if="item.balance==0 && item.point>0">{{item.point}} 积分</text>
										<text class="card-num" v-else-if="item.balance==0 && item.point==0 && item.goods_num>0">商品*{{item.goods_num}}</text>
										<text class="card-num" v-else-if="item.balance>0 && item.point>0 && item.goods_num==0">￥{{item.balance}}+{{item.point}}积分</text>
										<text class="card-num" v-else-if="item.balance>0 && item.point==0 && item.goods_num>0">￥{{item.balance}}+商品*{{item.goods_num}}</text>
										<text class="card-num" v-else-if="item.balance==0 && item.point>0 && item.goods_num>0">{{item.point}}积分+商品*{{item.goods_num}}</text>
									</view>
									<view class="card-amount">x{{item.num}}</view>
								</view>
								<view class="card-account">
									<span
										class="goods-num">共计{{item.num}}件商品</span><span>应付款：￥{{item.order_total_price}}</span>
								</view>
								<view class="button">
									<view class="button-left">
										<button type="primary" size="mini" @click="closeOrder(item.order_id)">关闭订单</button>
									</view>
						...
						<view class="card-item" v-else-if="item.status == 1">
							<view class="card-content">
								...
								<view class="button2">
									<view class="button3">
										<!-- <button type="primary" size="mini"  @click="orderDetail">查看订单</button> -->
									</view>
								</view>
							</view>
						</view>
				...
				<view class="card-no-data" v-else>
					<view class="card-image">
						<image src="https://cs.b2c.niuteam.cn/upload/uniapp/giftcard/detail/no_orders.png" mode=""></image>
					</view>
