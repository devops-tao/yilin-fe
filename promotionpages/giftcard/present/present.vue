<template>
	<view class="" >
		<view class="ns-present-bg"></view>

		<view class="present-body">
			<view class="card-item margin-bottom" :style="{background:$util.img(cardinfo.card_cover)?'url(' + $util.img(cardinfo.card_cover) + ') no-repeat 0 0/100% 100%':'url(https://cs.b2c.niuteam.cn/upload/uniapp/giftcard/detail/gitcard_default3.png) no-repeat 0 0/100% 100%' }">
				<view class="card-content">
					<view class="card-name">{{cardinfo.card_name}}</view>
					<view class="card-use">礼品卡权益</view>
					<!-- <view class="card-num">￥{{cardinfo.balance}} + {{cardinfo.point}}积分</view> -->
			...
			<view>
				...
				<textarea class="info" @input="oninput" maxlength="30" placeholder="请输入留言内容，最多30个字"
					placeholder-style="color:#b1b2b6; font-size:30rpx;"></textarea>
			</view>
		...
		<view class="launch-mask" v-if="show == true" @click="hide()"> 
			<view class="mask-img">
				<image :src="$util.img('https://cs.b2c.niuteam.cn/upload/uniapp/giftcard/detail/share.png')" mode="aspectFit"></image>
			</view>
		</view>
	...
	export default {
		...
		methods: {
					...
					if (this.$util.isWeiXin()) {
						this.$api.sendRequest({
							...
							success: res => {
								if (res.code == 0) {
									var wxJS = new Weixin();
									let url = this.$config.h5Domain+'/promotionpages/giftcard/present/accept?transfer_id=' + transfer_id;
									
									wxJS.setShareData({
										title: '送你一张礼品卡',
										desc: '快来领取吧，手慢就没有啦！',
										link: url,
										imgUrl: this.$util.img('https://cs.b2c.niuteam.cn/upload/uniapp/giftcard/detail/gift_bag3.png'),
									});
								}
							}
						});
			...
			copyUrl(id) {
				...
				if(this.transfer_id){
					let text = this.$config.h5Domain+'/promotionpages/giftcard/present/accept?transfer_id='+this.transfer_id;
					this.$util.copy(text, () => {
		...
		onShareAppMessage() {
			return {
				title: '送你一张礼品卡',
				imageUrl: this.$util.img('https://cs.b2c.niuteam.cn/upload/uniapp/giftcard/detail/gift_bag3.png'),
				path: '/promotionpages/giftcard/present/accept?transfer_id='+this.transfer_id,
				success: res => {},
				fail: res => {}
			};
		},
		onReady() {
			// #ifdef H5
				...
				// 			if (res.code == 0) {
				// 				var wxJS = new Weixin();
				// 				let url = this.$config.h5Domain+'/promotionpages/giftcard/present/accept?transfer_id=' + this.transfer_id;
