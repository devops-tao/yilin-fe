		...
		<mescroll-uni ref="mescroll" top="20" @getData="getMemberCounponList" v-if="token">
			<block slot="list">
				<view class="coupon-listone">
					<view class="item" v-for="(item, index) in list" :key="index" @click="toGoodsList(item)">
						<view class="item-left">
							<view class="item-flex">
								<view class="item-base">
									...
									<image v-else :src="$util.img('/upload/default/default_img/goods.png')" mode="aspectFit"></image>
								</view>
								<view class="item-info">
									<view class="use_title">
										<view class="title">{{item.divide_num}}名好友瓜分劵</view>
									</view>
									<view class="use_type">
										{{ item.name }}
									</view>
						...
						<view class="item-right">
							<view class="use_price"><text>￥</text>{{ item.money }}</view>
							<view class="item-btn">
								<view class="tag" @click="toGoodsList(item)">
									去瓜分
								</view>
							</view>
						</view>
				...
				<view v-if="!list.length && showEmpty" class="margin-top cart-empty" :fixed="!1">
					<ns-empty :isIndex="false" text="暂无数据"></ns-empty>
				</view>
			</block>
		</mescroll-uni>
