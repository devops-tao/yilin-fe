<template>
	<view class="guafen-box">
		<view v-if="groupInfoList.status!=2">
			<view class="guafen_adv" :style="{ backgroundImage: 'url(' + $util.img('/upload/uniapp/guafen/guafen_bg.png') + ')' }">
				<view class="explain">
				...
				<view class="guafen_content" v-if="groupInfoList == '' && guafenInfo.status == 1" :style="{ 'background-image': 'url(' + $util.img('/upload/uniapp/guafen/guafen_success_bg.png') + ')' }">
				...
				<view class="guafen_content" v-if="groupInfoList != ''&& guafenInfo.status == 1 &&groupInfoList.status==0||groupInfoList.status==2" :style="{ 'background-image': 'url(' + $util.img('/upload/uniapp/guafen/guafen_success_bg.png') + ')' }">
					...
					<view class="guafen_content_time">
						<image :src="$util.img('/upload/uniapp/guafen/bg_left.png')" mode=""></image>
						...
						<image :src="$util.img('/upload/uniapp/guafen/bg_right.png')" mode=""></image>
					</view>
					<view class="btn" @click="openSharePopup">邀请好友一起瓜分</view>
				</view>
				<block v-if="groupInfoList != ''&&groupInfoList.status==1 && g_member_state ==0">
					<view class="guafen_content-one" :style="{ 'background-image': 'url(' + $util.img('/upload/uniapp/guafen/guafen_success_bg.png') + ')' }">
				...
				<block v-if="groupInfoList.status!=1&&guafenInfo.status==-1 || groupInfoList.status!=1&&guafenInfo.status==2">
					...
					<view class="guafen_over" :style="{ 'background-image': 'url(' + $util.img('/upload/uniapp/guafen/guafen_success_bg.png') + ')' }">
						哎呀，来晚了，活动已结束
					</view>
				</block>
		...
		<uni-popup ref="rulePopup" type="center" :maskClick="false">
			<view class="rule-wrap">
				<view class="content-wrap">
					<image :src="$util.img('upload/uniapp/guafen/explain.png')" mode="" class="rule-head"></image>
		...
		<view @touchmove.prevent.stop  class="reward-popup">
			<uni-popup ref="linkPopup" type="center" :maskClick="false">
				<view class="warp-popup" v-if="inviterInfo">
					<view class="reward-wrap" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/guafen/guafen_popup.png') + ')' }">
				...
				<view class="warp-popup" v-else>
					<view class="reward-wrap" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/guafen/guafen_popup.png') + ')' }">
						<image :src="$util.img(groupInfoList.header_headimg ? $util.img(groupInfoList.header_headimg) : $util.getDefaultImage().default_headimg)" @error="groupInfoList.header_headimg = $util.getDefaultImage().default_headimg" class="img-head"></image>
		...
		<view @touchmove.prevent.stop  class="reward-popup-two">
			<uni-popup ref="linkSuccessPopup" type="center" :maskClick="false">
				<view class="reward-wrap-two" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/guafen/guafen_success.png') + ')' }">
		...
		<view @touchmove.prevent.stop  class="reward-popup-three">
			<uni-popup ref="friendsPopup" type="center" :maskClick="false">
				<view class="wrap-popup-three">
					<view class="reward-wrap-three" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/guafen/guafen_popup.png') + ')' }">
...
export default {
	...
	methods: {
		...
		toList(){
			...
			this.$util.redirectTo('/pages/goods/list/list', {goods_id_arr:arr});
		},
		toGuaFen(){
			this.$util.redirectTo('/promotionpages/guafen/guafen/list', {});
		},
		//去商品详情
		toDetail(item){
		...
		join(){
			if (!uni.getStorageSync('token')) {
				setTimeout(() => {
					this.$refs.login.open('/otherpages/hongbao/index/index?coupon_id=' + this.couponId +'&group_id='+this.groupId);
				});
			}else{
				this.$api.sendRequest({
		...
		getLaunch() {
			if (!uni.getStorageSync('token')) {
				setTimeout(() => {
					this.$refs.login.open('/promotionpages/guafen/guafen/index?coupon_id=' + this.couponId +'&group_id='+this.groupId);
				});
			}else{
				this.$api.sendRequest({
		...
		// 打开海报弹出层
		openPosterPopup() {
			this.$util.redirectTo('/promotionpages/guafen/guafen/poster', {coupon_id:this.couponId,group_id:this.groupId,inviter_id:this.guafenInfo.member_id},);
		},
		//生成海报
		getGoodsPoster() {
			//活动海报信息
		...
		copyUrl() {
			let text = this.$config.h5Domain + '/promotionpages/guafen/guafen/index';
	...
	onShareAppMessage(res) {
		var path ='/promotionpages/guafen/guafen/index?cid=' + this.guafenInfo.coupon_id + '&gid=' + this.groupInfoList.group_id + '&id=' + this.guafenInfo.member_id;
