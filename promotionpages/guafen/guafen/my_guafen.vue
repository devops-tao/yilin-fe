						...
						<view class="item-left">
							<view class="item-flex">
								<view class="item-base">
									...
									<image v-else :src="$util.img('/upload/uniapp/guafen/coupon_list_img.png')" mode="aspectFit"></image>
								</view>
						...
						<view class="item-right">
							...
							<view class="" v-else>
								<view class="image-lists" v-if="item.group_member_list.length>5">
									<image :src="item.group_member_list[0].headimg?$util.img(item.group_member_list[0].headimg):$util.getDefaultImage().default_headimg" @error="memberImageError()"></image>
									<image :src="item.group_member_list[1].headimg?$util.img(item.group_member_list[1].headimg):$util.getDefaultImage().default_headimg" @error="memberImageError()" class="posi-one"></image>
									<image :src="item.group_member_list[2].headimg?$util.img(item.group_member_list[2].headimg):$util.getDefaultImage().default_headimg" @error="memberImageError()" class="posi-two"></image>
									<image :src="item.group_member_list[3].headimg?$util.img(item.group_member_list[3].headimg):$util.getDefaultImage().default_headimg" @error="memberImageError()" class="posi-three"></image>
									<image :src="item.group_member_list[4].headimg?$util.img(item.group_member_list[4].headimg):$util.getDefaultImage().default_headimg" @error="memberImageError()" class="posi-four"></image>
									<view class="use_price color-base-border color-base-text posi" v-if="item.g_status==1||item.g_status==2"><text class="iconellipsis2 iconfont"></text></view>
									<view class="use_price color-base-border color-base-text posi" v-if="item.g_status==0">+</view>
								</view>
								<view class="img-list" v-else>
									<image :src="items.headimg ? $util.img(items.headimg) : $util.getDefaultImage().default_headimg" @error="memberImageError()" v-for="(items,indexs) in item.group_member_list" :key="indexs"></image>
...
<script>
	...
	export default {
		...
		onShow() {
			/* if (!this.addonIsExit.divideticket) {
				this.$util.showToast({
					title: '商家未开启好友瓜分券',
					mask: true,
					duration: 2000
			...
			if (uni.getStorageSync('token')) {
				...
				setTimeout(() => {
					this.$refs.login.open('/promotionpages/guafen/guafen/list');
				});
			}		
		},
		methods: {
			...
			memberImageError() {
				this.list.headimg = this.$util.getDefaultImage().default_headimg;
			},
			...
			imageError(index) {
				this.list[index].image = this.$util.getDefaultImage().default_goods_img;
				this.$forceUpdate();
			},
			toGoodsList(item) {
				this.$util.redirectTo('/promotionpages/guafen/guafen/index', {
