...
export default {
	...
	onShow() {
		...
		if (uni.getStorageSync('token')) {
			...
			this.$util.redirectTo('/otherpages/login/login/login', { back: '/otherpages/order/refund_detail/refund_detail?order_goods_id=' + this.order_goods_id });
		}
	},
	methods: {
		//联系客服
		goConnect() {
			...
			if (uni.getStorageSync('token')) {
				...
				if(this.kefuConfig.system == 0){
					...
					this.$util.redirectTo('wss://cs.b2c.niuteam.cn/wss');
				}
