	...
	export default{
		...
		methods:{
			...
			getGoodsInfo(){
				uni.getStorage({
					...
					success:res=>{
						...
						this.refund_data = refund_data.map(item=>{
							item.judge = true;
							return item;
						})
			...
			getOrderIdInfo(){
				...
				console.log('this.order_goods_id',this.order_goods_id)
			 ...
			 */
			next(){
				this.$util.redirectTo('/otherpages/order/refund_batch/refund_batch', { order_goods_id: this.order_goods_id.join(), refund_type:this.refund_type });
			}
		}
	}
</script>
