	...
	<view class="container" id="container" :data-theme="themeStyle">
		<view class="room" :class="keyWordsConfig.is_open == 1 ? 'active' : ''" @click="closePopWindow">
			...
			<view class="room-content-box">
				<view :id="'chat' + index" v-for="(item, index) in messageList" :key="index">
					<chat-message :message="item" :shopInfo="shopInfo" :userInfo="userInfo" @sendGood="sendGood($event, index)" @sendOrder="sendOrder($event, index)" :send="send"></chat-message>
				</view>
			</view>
		...
		<view class="input-content" :class="keyWordsConfig.is_open == 1 ? 'active' : ''">
			<view class="keyWords" v-if="keyWordsConfig.is_open == 1">
				<text v-for="(item, index) in keyWordsConfig.keyword_list" :key="index" @click="formData.content = item.keyword; sendMsg('message')">{{item.keyword}}</text>
			</view>
