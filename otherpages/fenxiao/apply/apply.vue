		...
		<view v-if="status === ''">
			...
			<view class="apply-wrap">
				...
				<view class="app-info-list" v-if="fenxiaoConfig.fenxiao_condition == 4">
					<view class="apply-message">
						<view class="apply-message-title color-base-bg-before">分销商申请条件</view>
						<text class="apply-message-info font-size-goods-tag">
							申请成为{{ fenxiaoWords.fenxiao_name }},需要购买以下指定商品(任选其一)
						</text>
					</view>
					...
					<view class="content-lists" v-if="isShow" @click="onOpen()">{{isOpen ? '展开更多' : '收起'}}
