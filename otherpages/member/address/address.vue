						...
						<view class="address-item" v-for="(item, index) in addressList" :key="index">
							...
							<view class="address-item-bottom">
								<view class="address-default">
									...
									<text class="default" v-else :class="{ 'color-base-text': item.is_default == 1 }" @click="setDefault(item.id)">默认地址</text>
	...
	export default {
		...
		onShow() {
			...
			if (uni.getStorageSync('token')) {
				...
				setTimeout(() => {
					this.$refs.login.open('/otherpages/member/address/address');
				});
			}
		},
		methods: {
