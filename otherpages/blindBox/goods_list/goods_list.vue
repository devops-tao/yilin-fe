<template>
	<view class="goods-list-box" :data-theme="themeStyle">
					...
					<view class="goods-list double-column" v-if="infoList.length>0">
						<view class="goods-item margin-bottom" v-for="(item,index) in infoList" :key="index" @click="toDetail(item)">
							<view class="goods-img">
								<image :src="goodsImg(item.goods_image)" mode="aspectFit" class="imgs-img" @error="infoListImg(index)"></image>
							</view>
							<view class="info-wrap">
								...
								<view class="discount-price">
									<text class="unit color-base-text font-size-tag">￥<text class="price color-base-text font-size-toolbar">{{item.price}}</text></text>
								</view>
							</view>
						</view>
					</view>
