<template>
	<view class="blind-box" :data-theme="themeStyle">
		<view class="null" v-if="info.blindbox_inventory<=0">
			<text class="iconfont icongantanhao1 gantan"></text><text>商品库存不足，请等待商家补货！</text>
		</view>
		<view class="box-bg" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/blindBox/index_bg.png') + ')' }">
			<view class="top">
				<view class="top-img">
				...
				<view class="right">
					...
					<view class="scribe-price font-size-tag">￥{{info.price}}</view>
					<view class="new-price font-size-tag" v-if="info.new_price>0">新人价：<text class="icon">￥</text><text class="money">{{info.new_price}}</text></view>
				</view>
				<view class="rule font-size-tag" @click="rule()">活动规则</view>
				<view class="explain-img" @click="toIndex()">
					<image :src="$util.img('/upload/uniapp/blindBox/index.png')" mode="" class="index-img"></image>
				</view>
			</view>
			<view class="cabinet-box">
				<view class="cabinet":style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/blindBox/cabinet_two.png') + ')' }">
						...
						<swiper-item  v-for="(item,index) in boxPage" :key="index">
							<view class="center-one">
								<view class="item" v-for="(item2,index2) in item" :key="index2" @click="checked(index2,item2)">
									<image v-if="item2.status==0" :src="$util.img('upload/uniapp/blindBox/box.png')" mode="" class="box"></image>
									<image v-if="item2.status==1" :src="$util.img('upload/uniapp/blindBox/null_box.png')" mode="" class="box"></image>
									<image :src="$util.img('upload/uniapp/blindBox/checked.png')" class="checked" v-if="act==index2"></image>
								</view>
							</view>
						</swiper-item>
					</swiper>
					<view class="btn-one" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/blindBox/btn_one.png') + ')' }" @click="changeGroup()">换一组</view>
					<view class="btn-two" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/blindBox/btn_two.png') + ')' }" @click="openBlind()">拆盲盒</view>
				</view>
			</view>
			<view class="list-box">
				<view class="list_bit" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/blindBox/goods_bg.png') + ')' }">
					<view class="list" id="innovate">
						<view class="imgs-list" ref="name" :style="{transform:'translateX('+lampX+'rpx)'}">
							<image v-for="(item,index) in infoList" :key="index" v-if="item.goods_image" :src="goodsImg(item.goods_image)" @click="redirect('/pages/goods/detail/detail',{sku_id: item.sku_id })" class="imgs-img" @error="infoListImg(index)" mode="aspectFit"></image>
						</view>
					</view>
				</view>
				<view class="all font-size-tag" @click="redirect('/otherpages/blindBox/goods_list/goods_list',{ blindbox_id: info.blindbox_id })">查看全部</view>
			</view>
		...
		<uni-popup ref="rulePopup" type="center">
			<view class="rule-wrap">
				<view class="content-wrap">
					<image :src="$util.img('upload/uniapp/blindBox/rule_bg.png')" mode="" class="rule-head"></image>
					<view class="rule-title font-size-toolbar">活动规则</view>
					<scroll-view scroll-y="true" class="rule">
						<view>
							<view class="rule-tit">
								<view class="rule-img">
									<image :src="$util.img('upload/uniapp/blindBox/tag.png')" mode=""></image>
								</view>
		...
		<view class="success-popup">
			<uni-popup ref="againPopup" type="center">
				<view class="success-wrap">
					<view class="content-wrap">
						<view class="success" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/blindBox/success_bg.png') + ')' }">
							<view class="goods">
								<view class="goods-img">
									<image v-if="boxDetail.sku_image" :src="$util.img(boxDetail.sku_image)" mode="aspectFit" class=""></image>
								</view>
								<view class="goods-name">{{boxDetail.sku_name}}</view>
								<view class="goods-price color-base-text"><text class="font-size-tag">￥</text>{{boxDetail.price}}</view>
							</view>
							<view class="again">
								<view class="color-sub discharge">
									已为您存放至盒柜<text class="color-base-text tobox" @click="redirect('/otherpages/blindBox/my_prize/my_prize',{ blindbox_id: boxDetail.blindbox_id })">前往我的奖品></text>
								</view>
								<view class="agains-btn">
									<view class="fill-btn" @click="redirect('/otherpages/blindBox/fillAddress/fillAddress',{blindbox_goods_id: boxDetail.blindbox_goods_id,out_trade_no:outTradeNo})">填写收货信息</view>
									<view class="again-btn color-base-bg" @click="reagain()">再来一次</view>
								</view>
							</view>
						</view>
						<view class="close-btn" @click="successClose()">
		...
		methods:{
			...
			openBlind(){
				if (!uni.getStorageSync('token')) {
					setTimeout(() => {
						this.$refs.login.open('/otherpages/blindBox/index/index?blindbox_id=' + this.blindbox_id);
					});
				}else{
			...
			toMyprize(){
				if (!uni.getStorageSync('token')) {
					...
					this.$util.redirectTo('/otherpages/blindBox/my_prize/my_prize',{ blindbox_id: this.info.blindbox_id })
				}
			},
			getBlindBox(){
				this.$api.sendRequest({
			...
			scrollWindow(){
				...
				query.select('#innovate').boundingClientRect(data => {
					width = data.width;
				}).exec()
			...
			infoListImg(index){
				this.infoList[index].goods_image =  this.$util.getDefaultImage().default_goods_img;
				this.$forceUpdate();
			},
			boxImg(){
				this.info.blindbox_images = this.$util.getDefaultImage().default_goods_img
				this.$forceUpdate();
			},
			redirect(url,id){
