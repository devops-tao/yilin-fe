<template>
	<view class="prize-box" :data-theme="themeStyle">
		<view class="my_prize" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/blindBox/index_bg.png') + ')' }">
			<view class="top">
				<view class="top-img">
				...
				<view class="right">
					...
					<view class="font-size-tag ul">
						<text class="li"></text><text class="">盲盒次数：</text><text class="">可拆{{blindgoods.blindbox_count}}次，剩余<text class="chi">{{blindgoods.blindbox_inventory}}</text>次</text>
					</view>
					<view class="font-size-tag ul">
						<text class="li"></text><text class="">我的次数：</text><text class="">已拆<text class="chi">{{blindgoods.buy_num}}</text>次</text>
					</view>
				</view>
			</view>
			<view class="content">
				<view class="tabs">
					<view :class="diff==1?'tab tab_left act':'tab tab_left'" @click="myPrize('1')">
						我的奖品
					</view>
					<view :class="diff==2?'tab tab_right act':'tab tab_right'" @click="myPrize('2')">
						参与人
					</view>
				</view>
						...
						<view class="big">
							<view class="list-tab" v-if="typeShow">
								<view class="goods-list" v-if="blindbox.length>0">
									<view class="list-body" v-for="(items,indexs) in blindbox" :key="indexs">
										...
										<view class="list-body-img-right"  v-if="diff==2">
											...
											<image v-else :src="$util.img('upload/uniapp/default_headimg.png')"></image>
										</view>
								...
								<view class="prize-null" v-else>
									<view class="prize-image" v-if="diff==1">
										<image :src="$util.img('upload/uniapp/blindBox/prize_null.png')" mode="aspectFill"></image>
									</view>
									<view class="prize-null-title" v-if="diff==1">暂无奖品~</view>
									<view class="prize-image" v-if="diff==2">
										<image :src="$util.img('upload/uniapp/blindBox/people_null.png')" mode="aspectFill"></image>
									</view>
									<view class="prize-null-title" v-if="diff==2">暂无参与人~</view>
								</view>
							</view>
							<view v-else>
								<view class="jiazai-box "style="background:#fff ;">
									<view class="jiazai">加载中...</view>
								</view>
							</view>
						</view>
					</block>
...
<script>
	...
	export default{
		...
		methods:{
			...
			errorImg(){
				this.blindgoods.blindbox_images = this.$util.getDefaultImage().default_goods_img
				this.$forceUpdate();
			},
			errorSkuImg(){
				this.blindbox.sku_image = this.$util.getDefaultImage().default_goods_img
			},
			errorSkuHeadImg(){
				this.blindbox.headimg = this.$util.getDefaultImage().default_headimg
			},
			deliver(e){
				this.$util.redirectTo('/otherpages/blindBox/fillAddress/fillAddress', {blindbox_goods_id:e.blindbox_goods_id,out_trade_no:e.out_trade_no},);
			},
			look(e){
				this.$util.redirectTo('/pages/order/list/list',{order_id:e.order_id});
			}
		}
	}
</script>
