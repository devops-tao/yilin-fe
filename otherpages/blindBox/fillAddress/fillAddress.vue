<template>
	<view class="fill-box" :data-theme="themeStyle">
		...
		<view class="send-mode" v-if="orderPaymentData.shop_goods_list.express_type.length > 1&&goodsInfo.is_virtual==0">
			...
			<view class="mode-btn">
				<button v-for="(deliveryItem, deliveryIndex) in orderPaymentData.shop_goods_list.express_type" :key="deliveryIndex" :type="deliveryItem.name == orderPaymentData.delivery.delivery_type ? 'primary' : 'default'" @click="selectDeliveryType(deliveryItem)" size="mini">
					{{ deliveryItem.title }}
				</button>
			</view>
		</view>
				...
				<view class="order-cell">
					...
					<view class="box">
						...
						<text v-if="deliveryWeek">
							<template v-if="timeInfo.showTime">（</template>{{ deliveryWeek }}<template v-if="timeInfo.showTime">）</template>
						</text>
						<text v-if="!canLocalDelicery">（休息中）</text>
					</view>
				</view>
				<picker v-if="timeInfo.showTimeBar && orderPaymentData.member_address && canLocalDelicery" :value="orderCreateData.buyer_ask_delivery_time" mode="time" @change="bindTimeChange">
					<view class="pick-block">
						<view class="font-size-base">送达时间</view>
						<view class="time-picker">
							<text>{{ orderCreateData.buyer_ask_delivery_time }}</text><text class="iconfont iconright cell-more"></text>
						</view>
					</view>
				</picker>
			</view>
		...
		<view class="store-box"  v-if="orderPaymentData.delivery.delivery_type == 'store'&&goodsInfo.is_virtual==0">
			<view v-if="storeInfo.currStore" @click="openSiteDelivery" class="store-info">
				<view v-if="storeInfo.currStore.store_image" class="icon img"><image v-if="storeInfo.currStore.store_image" :src="$util.img(storeInfo.currStore.store_image)" mode="aspectFill"></image></view>
		...
		<!-- 预填手机号 -->
		<view class="mobile-wrap" v-if="orderPaymentData.delivery.delivery_type == 'store' && storeInfo.currStore&&goodsInfo.is_virtual==0">
			<view class="form-group">
				<text class="require">*</text>
				<text class="text">预留手机</text>
				<input type="number" maxlength="11" placeholder="请输入您的手机号码" placeholder-class="color-tip placeholder" class="input font-size-base" v-model="member_address.mobile"/>
			</view>
		</view>
		<!-- 虚拟商品展示手机号 -->
		<view class="mobile-wrap" v-if="goodsInfo.is_virtual == 1">
		...
		<view class="primary">
			<button type="primary" class="" @click="get()">确定</button>
		</view>
					...
					this.$api.sendRequest({
						...
						success: res => {
							if (res.code >= 0) {
								...
								setTimeout(()=>{
									this.$util.redirectTo('/pages/order/list/list', {order_id:res.data});
								},2000)
								this.isSub = false;
							} else {
								this.isSub = false;
