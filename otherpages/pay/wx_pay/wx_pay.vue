<template>
	<view class="launch-mask" v-if="show == true">
		<view class="mask-img">
			<image :src="$util.img('upload/uniapp/invite_friends_share.png')" mode="aspectFit"></image>
		</view>
					...
					success: res => {
						if (res.code == 0) {
							if (res.data.pay_status == 2) {
								clearInterval(timer);
								that.$util.redirectTo('/otherpages/pay/result/result', { code: that.out_trade_no }, '', 'redirectTo');
							}
						} else {
							clearInterval(timer);
						}
