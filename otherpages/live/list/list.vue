	...
	<view class="live-content" :data-theme="themeStyle">
		...
		<mescroll-uni ref="mescroll" @getData="getListData">
			<block slot="list">
				<block v-if="liveList.length">
					<view class="live-wrap" v-for="(item, index) in liveList" :key="index" @click="entryRoom(item.roomid)">
						...
						<view class="room-info">
							<image :src="item.anchor_img != '' ? $util.img(item.anchor_img) : $util.getDefaultImage().default_headimg" class="anchor-img"></image>
				...
				<block v-else>
					<ns-empty text="暂无直播记录" :isIndex="isIndex"></ns-empty>
				</block>
			</block>
		</mescroll-uni>
