<template>
	<view class="guafen-box">
		<view class="" v-if="groupInfoList.status!=2">
			<view class="guafen_adv" :style="{ backgroundImage: 'url(' + $util.img('/upload/uniapp/hongbao/hongbao_bg.png') + ')' }">
				<view class="explain">
					<view class="explain-img" @click="toIndex()">
						<image :src="$util.img('/upload/uniapp/hongbao/index.png')" mode="" class="index-img"></image>
					</view>
				...
				<view class="guafen_content" v-if="groupInfoList == '' && hongbaoInfo.status == 1">
					...
					<view class="guafen_content_num">{{hongbaoInfo.money|int}}<text>元</text></view>
				...
				<view class="guafen_content" v-if="groupInfoList != ''&& hongbaoInfo.status == 1 &&groupInfoList.status==0||groupInfoList.status==2">
						...
						<scroll-view :scroll-y="true" :show-scrollbar="false" v-if="groupInfoList.member_list.length > 0">
							<view class="hongbao-lists">
								<view class="guafen_header_img" v-for="(headerItem,headerIndex) in groupInfoList.member_list" :key="headerItem.member_id">
									...
									<image v-else :src="$util.img('/upload/default/default_img/head.png')" mode=""></image>
								</view>
					...
					<view class="guafen_content_title">仅差<text class="guafen_content_color">{{groupInfoList.num - groupInfoList.member_list.length}}</text>人，即可瓜分<text class="guafen_content_color">{{hongbaoInfo.money|int}}</text>元现金红包</view>
					<view class="guafen_content_time">
						<image :src="$util.img('/upload/uniapp/hongbao/bg_left.png')"></image>
						...
						<image :src="$util.img('/upload/uniapp/hongbao/bg_right.png')"></image>
					</view>
			...
			<view class="guafen-how">
				<view class="how-title font-size-toolbar">
					如何瓜分红包
				</view>
				<view class="step">
					<view class="step-img">
						<image :src="$util.img('/upload/uniapp/hongbao/active.png')" class="step-one"></image>
						<image :src="$util.img('/upload/uniapp/hongbao/jiantou.png')" class="step-jiantou"></image>
						<image :src="$util.img('/upload/uniapp/hongbao/friend.png')" class="step-one"></image>					
						<image :src="$util.img('/upload/uniapp/hongbao/jiantou.png')" class="step-jiantou"></image>
						<image :src="$util.img('/upload/uniapp/hongbao/reward.png')" class="step-three"></image>
					</view>
					<view class="step-txt">
			...
			<view class="rule-description">
				<view class="rule-description-title font-size-toolbar">
					活动说明
				</view>
				<view class="color-sub remark">
					{{ hongbaoInfo.remark }}
				</view>
			</view>
		</view>
		...
		<uni-popup ref="rulePopup" type="center" :maskClick="false">
			<view class="rule-wrap">
				<view class="content-wrap">
					<image :src="$util.img('upload/uniapp/hongbao/rule_head.png')" mode="" class="rule-head"></image>
		...
		<view @touchmove.prevent.stop  class="reward-popup">
			<uni-popup ref="linkPopup" type="center" :maskClick="false">
				<view class="wrap-popup" v-if="inviterInfo">
					<view class="reward-wrap" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/hongbao/popup_bg.png') + ')' }">
						<image :src="$util.img(inviterInfo.headimg ? $util.img(inviterInfo.headimg) : $util.getDefaultImage().default_headimg)" @error="inviterInfo.headimg = $util.getDefaultImage().default_headimg" class="img-head"></image>
						<view class="nickname">{{inviterInfo.nickname|guolv}}</view>
						<view class="txt">邀请你加入团队</view>
						<view class="guafen-hongbao">瓜分现金红包</view>
						<view class="qu-btn" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/hongbao/btn_bg.png') + ')' }" @click="join()">加入队伍</view>
					</view>
					<view class="close-btn" @click="closeLinkPopup()">
						<image :src="$util.img('/upload/uniapp/hongbao/close.png')" mode=""></image>
					</view>
				</view>
		...
		<view @touchmove.prevent.stop  class="reward-popup-three">
			<uni-popup ref="friendsPopup" type="center" :maskClick="false">
				<view class="wrap-popup-three">
					<view class="reward-wrap-three" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/hongbao/popup_bg.png') + ')' }">
						<view class="join-three">你已加入<text class="nickname-three">{{groupInfoList.header_nickname|guolv}}</text>的瓜分团</view>
						<view class="guafen-three" v-if="groupInfoList.member_list">仅差<text>{{groupInfoList.num - groupInfoList.member_list.length}}</text>人即可瓜分现金红包</view>
						<view class="txt-three">{{hongbaoInfo.money|int}}元</view>
						<view class="qu-btn-three" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/hongbao/btn_bg.png') + ')' }" @click="goFriend()">喊好友一起瓜分</view>
					</view>
		...
		<view @touchmove.prevent.stop  class="reward-popup-two">
			<uni-popup ref="linkSuccessPopup" type="center" :maskClick="false">
				<view class="wrap-popup-two">
					<view class="reward-wrap-two" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/hongbao/popup_bg.png') + ')' }">
						<view class="title-two">瓜分红包</view>
						<view class="together-two">- 好友已集齐 -</view>
						<view class="txt-two">{{hongbaoInfo.money|int}}元</view>
						<view class="qu-btn-two" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/hongbao/btn_bg.png') + ')' }" @click="closeGetItNow()">立即瓜分红包</view>
					</view>
				</view>
			</uni-popup>
		</view>
...
export default {
	...
	onShareAppMessage(res) {
		var path ='/otherpages/hongbao/index/index?hid=' + this.hongbaoInfo.hongbao_id +'&gid=' + this.groupInfoList.group_id+'&id='+this.hongbaoInfo.member_id;
		if (this.memberId) path += '&source_member=' + this.memberId;
		return {
			title: '还差' + (this.groupInfoList.num - this.groupInfoList.member_list.length) + '人瓜分成功',
			path: path,
			success: res => {console.log(path,'00')},
			fail: res => {console.log(path,'11')}
		};
	},
	methods: {
		getMemberId() {
		...
		toLook(){
			this.$util.redirectTo('/otherpages/member/balance_detail/balance_detail', {group_id:this.groupInfoList.group_id,from_type:'hongbao'});
		},
		//去瓜分新红包
		toGuaFen(){
			this.$util.redirectTo('/otherpages/hongbao/list/list', {});
		},
		toIndex(){
		...
		join(){
			if (!uni.getStorageSync('token')) {
				setTimeout(() => {
					this.$refs.login.open('/otherpages/hongbao/index/index?hongbao_id=' + this.hongBaoId +'&group_id='+this.groupId+'&inviter_id='+this.inviterId);
				});
			}else{
				this.$api.sendRequest({
		...
		getLaunch() {
			if (!uni.getStorageSync('token')) {
				setTimeout(() => {
					this.$refs.login.open('/otherpages/hongbao/index/index?hongbao_id=' + this.hongBaoId +'&group_id='+'');
				});
			}else{
				this.$api.sendRequest({
		...
		// 打开海报弹出层
		openPosterPopup() {
			this.$util.redirectTo('/otherpages/hongbao/poster/poster', {hongbao_id:this.hongBaoId,group_id:this.groupId,inviter_id:this.hongbaoInfo.member_id},);
		},
		// 关闭海报弹出层
		closePosterPopup() {
		...
		copyUrl() {
			let text = this.$config.h5Domain + '/otherpages/hongbao/index/index';
		...
		headerimageError(index) {
			this.list[index].headimg = this.$util.getDefaultImage().default_headimg;
			this.$forceUpdate();
		},
		onPageHide() {
...
.guafen_adv {
	background-image: url(https://cs.b2c.niuteam.cn//upload/uniapp/guafen/guafen_bg.png);
	background-size: 100%;
	background-repeat: no-repeat;
	padding: 20rpx 30rpx;
	image {
