...
export default {
	...
	onLoad(option) {
		...
		if (this.$util.isWeiXin() && !option.code) {
			let redirect_url = this.$config.h5Domain + '/otherpages/login/login/login';
	...
	methods: {
		 ...
		 */
		toRegister() {
			if (this.back) this.$util.redirectTo('/otherpages/login/register/register', { back: this.back });
			else this.$util.redirectTo('/otherpages/login/register/register');
		},
		/**
		 * 忘记密码
		 */
