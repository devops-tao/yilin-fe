					...
					<view>
						<block v-if="manjian">
							<view class="order-cell" style="align-items: baseline;">
								<view class="tit">
									<text class="promotion-mark ns-gradient-promotionpages-payment" v-if="post_free.freeshipping">包邮</text>
									<text class="promotion-mark ns-gradient-promotionpages-payment" v-else>{{manjian.manjian_name}}</text>
								</view>
								<view class="promotion-content"  v-if="post_free.freeshipping">
									<view class="tit tit-content">满{{post_free.freeshipping.price}}包邮</view>
								</view>
								<view class="promotion-content" v-else>
									<view class="tit tit-content">{{manjian.manjian}}</view>
									<view class="tit tit-content">{{manjian.point_text}}</view>
									<view class="tit tit-content">{{manjian.mansong}}</view>
								</view>

							</view>
							
						</block>
						<view class="order-cell" v-if="manjian.free_shipping != undefined">
							<text class="tit">
								<text class="promotion-mark ns-gradient-promotionpages-payment">包邮</text>
							</text>
							<view class="tit" style="max-width: 540rpx;font-size: 12px;line-height: 35rpx;margin-bottom: 10rpx;">{{ manjian.free_shipping }}</view>
						</view>
					</view>
				</scroll-view>
				<view class="popup-footer" :class="{ 'bottom-safe-area': isIphoneX }">
