						...
						<view class="order-footer">
							...
							<view class="order-action" v-if="orderItem.action.length > 0">
								<view class="order-time" v-if="orderItem.order_status == 0" id="action-date">
									<image :src="$util.img('/upload/uniapp/order/time.png')"></image>
									剩余时间：
									<uni-count-down
...
export default {
	...
	onShow() {
		...
		if (uni.getStorageSync('token')) {
			...
			setTimeout(() => {
				this.$refs.login.open('/pages/order/list/list?status=' + this.orderStatus);
			});
		}
	},
	methods: {
		...
		operation(action, orderData) {
			...
			switch (action) {
				...
				case 'memberTakeDelivery': //收货
					...
					break;
				case 'trace': //查看物流
					this.$util.redirectTo('/otherpages/order/logistics/logistics', {
