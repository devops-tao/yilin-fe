<template>
	<view class="detail-container" :class="{ 'safe-area': isIphoneX }" :data-theme="themeStyle">
		...
		<view class="status-wrap color-base-bg" :style="{ backgroundImage: 'url(' + $util.img('upload/uniapp/order/status-wrap-bg.png') + ')' }">
			<view class="order-status-left">
				...
				<view class="status-name">
					...
					<view class="desc" v-if="orderData.promotion_type == 'presale' && orderData.order_status == 1">预计{{ $util.timeStampTurnTime(orderData.predict_delivery_time, true) }}日后发货</view>
				</view>
			</view>
		</view>
		...
		<view class="order-money">
			...
			<view class="order-action fixed-bottom" :class="{ 'bottom-safe-area': isIphoneX }" v-if="orderData.action.length > 0">
				...
				<!-- <view class="order-box-btn white" v-if="orderData.refund_batch_status" @click="operation('refundbatch')">
					<text>批量退款</text>
				</view> -->
			</view>
			
			<view
				class="order-action fixed-bottom"
			...
			<view class="order-action fixed-bottom" :class="{ 'bottom-safe-area': isIphoneX }" v-else>
				<!-- <view class="order-box-btn white" v-if="orderData.refund_batch_status" @click="operation('refundbatch')">
					<text>批量退款</text>
				</view> -->
			</view>
...
export default {
	...
	onShow() {
		...
		if (uni.getStorageSync('token')) {
			...
			this.$util.redirectTo('/otherpages/login/login/login', { back: '/pages/order/detail/detail?order_id=' + this.orderId });
		}
		this.getKekuConfig();
	},
	methods: {
		...
		goConnect() {
			if (uni.getStorageSync('token')) {
				...
				this.$refs.login.open('/pages/order/detail/detail?order_id=' + this.orderId);
				return;
			}
		},
		getKekuConfig() {
		...
		operation(action) {
			switch (action) {
				...
				case 'memberTakeDelivery': //收货
					...
					break;
				case 'trace': //查看物流
					this.$util.redirectTo('/otherpages/order/logistics/logistics', { order_id: this.orderData.order_id });
					break;
				case 'memberOrderEvaluation': //评价
					this.$util.redirectTo('/otherpages/order/evaluate/evaluate', { order_id: this.orderData.order_id });
					break;
				case 'refundbatch':
					this.$util.redirectTo('/otherpages/order/refund_type_select/refund_type_select', { order_goods_id: this.orderData.refund_order_goods_ids.toString() });
					break;
			}
		},
		imageError(index) {
